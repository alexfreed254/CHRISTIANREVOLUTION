    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: index * 0.05 }}
      ...
      <Link to={`/media/${media.id}`} className="block focus:outline-none focus-visible:ring-2 focus-visible:ring-crm-purple rounded-xl">
        <div className="relative aspect-video rounded-xl overflow-hidden bg-crm-dark shadow-lg shadow-slate-200">
          ...
          {media.is_new && !media.is_live && (
            <div className="absolute top-2 left-2 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-white"
        ...
        <div className="mt-3 flex gap-3">
          <div className="w-9 h-9 rounded-full bg-gradient-to-br from-crm-purple to-crm-purple-light flex items-center justify-center text-white text-sm font-bold flex-shrink-0">
      ...
      <motion.div initial={false} animate={{ opacity: isHovered ? 1 : 0, y: isHovered ? 0 : 5 }}
        ...
        <button onClick={(e) => { e.preventDefault(); onPlayAudio?.(media); }}
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-xs text-crm-gray-light hover:text-crm-purple-dark transition-all">
          <Play className="w-3 h-3" /> Audio
        </button>
        <button className="p-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-crm-gray-light hover:text-crm-purple-dark transition-all"><Heart className="w-3.5 h-3.5" /></button>
        <button className="p-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-crm-gray-light hover:text-crm-purple-dark transition-all"><Share2 className="w-3.5 h-3.5" /></button>
        <button className="p-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-crm-gray-light hover:text-crm-purple-dark transition-all"><Download className="w-3.5 h-3.5" /></button>
      </motion.div>
    </motion.div>
  )
}
