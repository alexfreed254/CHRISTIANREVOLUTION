...
export default function MiniAudioPlayer() {
  ...
  return (
    <>
      ...
      <motion.div initial={{ y: 100 }} animate={{ y: 0 }} className="fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-xl border-t border-slate-200 shadow-lg">
        <div ref={progressRef} className="h-1 bg-slate-200 cursor-pointer group/progress" onClick={handleSeek}>
        ...
        <div className="max-w-7xl mx-auto px-4 py-2 flex items-center gap-3">
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            <button onClick={prevTrack} className="p-2 rounded-lg hover:bg-slate-200 text-crm-gray-light hover:text-crm-purple-dark transition-all"><SkipBack className="w-4 h-4" /></button>
            ...
            <button onClick={nextTrack} className="p-2 rounded-lg hover:bg-slate-200 text-crm-gray-light hover:text-crm-purple-dark transition-all"><SkipForward className="w-4 h-4" /></button>
          </div>
          <div className="hidden md:flex items-center gap-3">
            <span className="text-xs text-crm-gray font-mono">{formatTime(progress)} / {formatTime(duration)}</span>
            <div className="flex items-center gap-1 group/volume">
              <button onClick={() => setVolume(volume === 0 ? 0.8 : 0)} className="p-1.5 rounded hover:bg-slate-200 text-crm-gray-light">
          ...
          <div className="flex items-center gap-1">
            <div className="relative">
              <button onClick={() => setShowSpeedMenu(!showSpeedMenu)}
                className="px-2 py-1 rounded-lg bg-slate-100 text-xs text-crm-gray-light hover:bg-slate-200 transition-all">{playbackSpeed}x</button>
              <AnimatePresence>
                {showSpeedMenu && (
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }}
                    className="absolute bottom-full right-0 mb-2 w-20 bg-crm-dark border border-slate-200 rounded-xl overflow-hidden shadow-2xl">
                    {speeds.map((s) => (
                      <button key={s} onClick={() => { setPlaybackSpeed(s); setShowSpeedMenu(false); }}
                        className={`w-full px-3 py-2 text-xs transition-all ${playbackSpeed === s ? 'bg-crm-purple/20 text-crm-purple' : 'text-crm-gray-light hover:bg-slate-100'}`}>
                        {s}x
                      </button>
                    ))}
                  </motion.div>
            ...
            <button onClick={() => setIsExpanded(!isExpanded)} className="p-2 rounded-lg hover:bg-slate-200 text-crm-gray-light transition-all">
      ...
      <AnimatePresence>
        {isExpanded && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            className="fixed inset-0 z-50 bg-white/95 backdrop-blur-xl flex items-center justify-center p-4" onClick={() => setIsExpanded(false)}>
            <motion.div initial={{ scale: 0.9, y: 50 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 50 }}
              ...
              <div className="h-2 bg-slate-200 rounded-full cursor-pointer mb-2" onClick={handleSeek}>
              ...
              <div className="flex items-center justify-center gap-6">
                <button onClick={prevTrack} className="p-3 rounded-full hover:bg-slate-200 text-crm-gray-light"><SkipBack className="w-6 h-6" /></button>
                <motion.button whileTap={{ scale: 0.9 }} onClick={togglePlay}
                ...
                <button onClick={nextTrack} className="p-3 rounded-full hover:bg-slate-200 text-crm-gray-light"><SkipForward className="w-6 h-6" /></button>
              </div>
              <div className="flex items-center justify-center gap-4 mt-8">
                <button className="flex items-center gap-2 px-4 py-2 rounded-full bg-slate-100 hover:bg-slate-200 text-sm text-crm-gray-light transition-all"><Download className="w-4 h-4" /> Download</button>
                <button className="flex items-center gap-2 px-4 py-2 rounded-full bg-slate-100 hover:bg-slate-200 text-sm text-crm-gray-light transition-all"><ListMusic className="w-4 h-4" /> Queue</button>
                <button className="flex items-center gap-2 px-4 py-2 rounded-full bg-slate-100 hover:bg-slate-200 text-sm text-crm-gray-light transition-all"><Clock className="w-4 h-4" /> Sleep Timer</button>
              </div>
            </motion.div>
          </motion.div>
        )}
